package org.asymetryk.modular.verification;
...
class ModularVerifierTest {
	...
	void testing_an_isolated_module_should_return_success(){
		ModularVerifier modularVerifier = new ModularVerifier("org.asymetryk.modulyth.testedmodules.validisolatedmod");
	...
	void testing_a_module_hierarchy_shoud_return_an_error(){
		ModularVerifier modularVerifier = new ModularVerifier("org.asymetryk.modulyth.testedmodules.invalidhierarchy");
		ModularVerifier.Result result = modularVerifier.verify();
	...
	void testing_module_with_multiple_nested_modules_should_report_first_violation() {
		...
		Assertions.assertThat(invalid.parentModuleName()).isEqualTo("parent");
		// Should report one of the children (implementation-dependent which one)
		Assertions.assertThat(invalid.nestedModuleName()).isIn("child1", "child2");
	}
	...
	void testing_module_using_exported_class_should_return_success() {
		ModularVerifier modularVerifier = new ModularVerifier(
			"org.asymetryk.modulyth.testedmodules.modulea",
			"org.asymetryk.modulyth.testedmodules.modulevalidtest"
		);
		ModularVerifier.Result result = modularVerifier.verify();
	...
	void testing_module_using_non_exported_class_should_return_error() {
		ModularVerifier modularVerifier = new ModularVerifier(
			"org.asymetryk.modulyth.testedmodules.modulea",
			"org.asymetryk.modulyth.testedmodules.moduleusingnotexported"
		);
		ModularVerifier.Result result = modularVerifier.verify();
